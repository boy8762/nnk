              ...
              (item, index) =>
                item.href && (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      'relative inline-block text-sm font-medium text-foreground/60 transition-all hover:text-foreground hover:after:w-full',
                      path === item.href && 'font-bold text-foreground',
                      item.disabled && 'cursor-not-allowed opacity-80',
                    )}
                    onClick={() => handleChangeStatusOpen(false)}
                  >
                    {item.title}
                    {/* Underline animation with pseudo-element */}
                    <span className="absolute bottom-0 left-0 h-0.5 w-0 bg-foreground transition-all duration-300 ease-in-out group-hover:w-full" />
                  </Link>
                )
            )}
          </nav>
